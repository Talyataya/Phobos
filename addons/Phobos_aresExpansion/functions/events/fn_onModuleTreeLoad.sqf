...
// Author: Anton Struyk
// Modified by Talya
// Taken from: Ares Mod(v1.8.1) - fn_onModuleTreeLoad.sqf
//
//////////////////////////////////////////////////////////////////////////
...
{
	// Only add modules that define themselves in the "Phobos" category
	_moduleCategory = gettext (configFile >> "CfgVehicles" >> _x >> "category");
	if(_moduleCategory == "Phobos") then
	{
...
} forEach getArray (configFile >> "cfgPatches" >> "Phobos" >> "units");
...
// Wait for Ares modules to load to avoid wrong subCategory generations.
sleep 0.05;
...
_categoryBranches = [];
{
	_tvText = _x;
	_tvIcon = "";
	switch ([] call Phobos_fnc_getParentMod) do {
	case "Ares": {_tvIcon = "\ares_zeusExtensions\data\icon_ares.paa";};
	case "Achilles": {_tvIcon = "\achilles\data_f_ares\icons\icon_ares.paa";};
	};
