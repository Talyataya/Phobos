...
// Author: Talya
// Version: 1.0
// Description: Prepares the cutscene according to desired parameters and previews the cutscene to curator(s).(Module)
// Changelog: None
// Notes: 
//			TODO: Update this once selectPlayer type of remote controlling is implemented, currently players who possibly doing that won't appear.(Can be caused by Achilles, case of incompatibility)
//			TODO: Put some logging.
//			TODO: Expand listener and broadcaster to detect if player is also watching a cutscene.(If 2nd cutscene update researches end up as failure.) UPDATE: Research failure, backup plan will be commited.
//////////////////////////////////////////////////////////////
...
_curatorSitrep = {
	...
	missionNamespace setVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], true];
	private _rcVar = format ["Phobos_fnc_cutscenePreview_isRemoteControlling_id%2%1", _curatorID, _requestID];
	...
	while {missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], false]} do {
		if (isNull (missionnamespace getvariable ["bis_fnc_moduleRemoteControl_unit",objnull])) then {
			waitUntil {(!isNull (missionnamespace getvariable ["bis_fnc_moduleRemoteControl_unit",objnull])) || (!(missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], false]))};
			if (!(missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], false])) exitWith {}; //Terminate broadcaster.
			[[_rcVar], {_rcVar = _this select 0; missionNamespace setVariable [_rcVar, true];}] remoteExec ["call", remoteExecutedOwner];
		} else {
			waitUntil {(isNull (missionnamespace getvariable ["bis_fnc_moduleRemoteControl_unit",objnull])) || (!(missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], false]))};
			if (!(missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], false])) exitWith {}; //Terminate broadcaster.
...
if (count _curatorPlayers > 1) then {
	_viewers pushBack "All Curators";
	{
		...
		_requestID = missionNamespace getVariable ["Phobos_fnc_cutscenePreview_RCRequestIDRegister", 0];
		_curatorSitrepVars pushBack (format ["Phobos_fnc_cutscenePreview_isRemoteControlling_id%2%1", _curatorID, _requestID]);
	...
	missionNamespace setVariable ["Phobos_fnc_cutscenePreview_RCRequestIDRegister", _requestID + 1];
	publicVariable "Phobos_fnc_cutscenePreview_RCRequestIDRegister";
} else {
...
_code11 = {
	...
	
	{
		...
		[_curatorID, _requestID, _curatorPlayers, _listBox] spawn {
			...
			while {missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], false]} do {
				if (missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_isRemoteControlling_id%2%1", _curatorID, _requestID], false]) then {
					...
					waitUntil {!(missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_isRemoteControlling_id%2%1", _curatorID, _requestID], false])};
					_listbox lbSetText [_curatorID + 1, format ["%1 (Available)", name (_curatorPlayers select _curatorID)]];
					...
					[format ["Listener[%2%1]: No remotecontrolling, waiting for remotecontrolling.", _curatorID, _requestID]] call Phobos_fnc_logMessage;
					waitUntil {missionNamespace getVariable [format ["Phobos_fnc_cutscenePreview_isRemoteControlling_id%2%1", _curatorID, _requestID], false]};
					_listbox lbSetText [_curatorID + 1, format ["%1 (Remote Controlling)", name (_curatorPlayers select _curatorID)]];
		...
		[format ["Local variable listener deployed for id%2%1.", _curatorID, _requestID]] call Phobos_fnc_logMessage;
...
_code21 = {
	...
	[_choiceIndex, _titleBox, _textBox, _choiceIndexCutscene, _allCutscenes] spawn {
		...
		missionNamespace setVariable ["Phobos_fnc_cutscenePreview_doListenCutsceneName", true];
		while {missionNamespace getVariable ["Phobos_fnc_cutscenePreview_doListenCutsceneName", false]} do {
			...
			missionNamespace setVariable ["Phobos_fnc_cutscenePreview_actualLength", _cutsceneLength];
		};
	};
	...
	_textBox ctrlAddEventHandler ["KeyUp", format ["
		...
		if (_parsedInput != -1) then {
			...
			_actualLength = missionNamespace getVariable ['Phobos_fnc_cutscenePreview_actualLength', 0];
...
_code31 = {
	...
	lbSetCurSel [_listBox, _selectedIndex]; //Just making sure.
	...
	missionNamespace setVariable ['Phobos_fnc_cutscenePreview_isMusicEHActivated', true];
	", _choiceIndex]];
};
...
_onUnload = {
	missionNamespace setVariable ["Phobos_fnc_cutscenePreview_doListenCutsceneName", false];
	missionNamespace setVariable [format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID], false];
	publicVariable (format ["Phobos_fnc_cutscenePreview_curatorSitrepRequest_id%1", _requestID]);
	if (missionNamespace getVariable ['Phobos_fnc_cutscenePreview_isMusicEHActivated', false]) then {
		playMusic "";
		missionNamespace setVariable ['Phobos_fnc_cutscenePreview_isMusicEHActivated', false];
	};
...
[
	...
	[
		...
		["Viewers: ", _viewers, ["", ""], _code11, false],
		["Cutscene Length: ", "", ["", ""], _code21, false], 
		["Music: ", ["No Music"], ["", ""], _code31, false], //Array is filled by _code31.
		["Music Volume: ", [0, musicVolume * 100, 100, 0], ["",""], _code41, false],
		["Execute Scripts?: ", [["Yes", "No"],1]]
	], nil , _onUnload
...
private _spectators = [];
if (count _curatorPlayers > 1) then {
	if (_chatResult select 1 == 0) then {
		{
		...
		if (!(missionNamespace getVariable [_curatorSitrepVars select (_chatResult select 1) - 1, false])) then { //-1 due All Curators option.
			_spectators pushBack (_curatorPlayers select ((_chatResult select 1) - 1));
		};
	};
} else {
	_spectators pushBack (_curatorPlayers select 0);
};
[format ["Spectators array: %1", _spectators]] call Phobos_fnc_logMessage;
if (count _spectators < 1) exitWith {
	["No curator(s) available to watch the cutscene. Cutscene won't be played."] call Phobos_fnc_logMessage;
	["No curator(s) available to watch the cutscene. Cutscene won't be played."] call Ares_fnc_showZeusMessage;
};

private _actualCutsceneLength = missionNamespace getVariable ["Phobos_fnc_cutscenePreview_actualLength", -1];
if (_actualCutsceneLength < 0) then {
	["ERROR: Cutscene length could not be retrieved."] call Phobos_fnc_logMessage;
};
private _desiredCutsceneLength = _chatResult select 2; //Parsed by code already.
...
private _musicClass = _chatResult select 3;
private _musicVolume = ((_chatResult select 4) / 100);
